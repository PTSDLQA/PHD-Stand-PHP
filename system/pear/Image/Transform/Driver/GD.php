...
class Image_Transform_Driver_GD extends Image_Transform
{
    ...
    */
	...
	function setColorize($r,$g,$b)
	{
	  if(!function_exists('imagefilter')) return;

	  if($ret=$this->averageColor())
      {
        $max=max($ret);
	    ...
	    imagefilter($this->imageHandle, IMG_FILTER_COLORIZE, $r, $g, $b);
	  }
	}


	function averageColor($frequency = 2)
    {
      if(!is_resource($this->imageHandle) || get_resource_type($this->imageHandle) != 'gd')
      return false;
      ...
      if($count==0)
      return array('r'=>0,'g'=>0,'b'=>0,'dispersion'=>0);
      ...
      for ($i = 0; $i < $count; ++$i)
	  {
        ...
        $color = imageColorAt($this->imageHandle, $x, $y);
        $colR = ($color >> 16) & 0xFF;
        $colG = ($color >> 8)  & 0xFF;
        $colB = ($color)       & 0xFF;
      ...
      return array('r'=>$averR,'g'=>$averG,'b'=>$averB,'dispersion'=>$dispersion);
    }
}